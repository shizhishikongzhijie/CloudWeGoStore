...
func (p *EchoEchoArgs) fastWriteField1(buf []byte, binaryWriter bthrift.BinaryWriter) int {
	offset := 0
	offset += bthrift.Binary.WriteFieldBegin(buf[offset:], "r", thrift.STRUCT, 1)
	offset += p.R.FastWriteNocopy(buf[offset:], binaryWriter)
	offset += bthrift.Binary.WriteFieldEnd(buf[offset:])
	return offset
}
...
func (p *EchoEchoArgs) field1Length() int {
	l := 0
	l += bthrift.Binary.FieldBeginLength("r", thrift.STRUCT, 1)
	l += p.R.BLength()
	l += bthrift.Binary.FieldEndLength()
	return l
}
